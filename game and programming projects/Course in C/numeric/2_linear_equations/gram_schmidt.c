...
for(int i=0; i<m; i++){
	...
	for(int j=0; j<n; j++){
		a_i_norm_square += pow( gsl_matrix_get(A,j,i),2 );   //bemærk at i,j bytts om pga. dum notation mht. søjle vs række
		}
	...
	for(int j=i+1; j<m; j++){
		...
		for(int k=0; k<n; k++){
			dot_product_ij += gsl_matrix_get(A,k,i)*gsl_matrix_get(A,k,j);   //bemærk at i,k og k,j byttes om
			}
		gsl_matrix_set(R,i,j,dot_product_ij);


		for(int k=0; k<n; k++){
			gsl_matrix_set(A,k,j, gsl_matrix_get(A,k,j) - gsl_matrix_get(A,k,i)*dot_product_ij );  //bemærk at i,k og k,j byttes om
