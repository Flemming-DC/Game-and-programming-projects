...
void numeric_gradient(double beta(gsl_vector*, gsl_vector*, gsl_vector*)
                        , gsl_vector* x, gsl_vector* d1, gsl_vector* d2, gsl_vector*grad){
        double fx=beta(x,d1,d2);
        double eps=1e-6;
        for(int i=0;i< x->size;i++){
                ...
                gsl_vector_set(grad,i,(beta(x,d1,d2)-fx)/dx);
...
void qnewton(double beta(gsl_vector*, gsl_vector*, gsl_vector*)
                , gsl_vector*x, gsl_vector* d1, gsl_vector* d2, double acc) {
        ...
        numeric_gradient(beta,x,d1,d2,gradient);
        double fx=beta(x,d1,d2),fz;
        double eps=1e-6;
        int max_nstep=1*1e6;
        ...
        while(nsteps<max_nstep){
                ...
                numeric_gradient(beta,z,d1,d2,gz);
...
if(nsteps==max_nstep) printf("warning: maximum step number reached. minimization is incomplete.\n");
